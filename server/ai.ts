...
function persistCall(
  ...
  // 确保表存在（兼容服务进程早于 schema 变更启动的情况）
  getDb()
  const payload = buildArchivePayload(input)
  try {
    if (callId != null) {
      updateAiCall(callId, payload)
      return callId
    }
    return insertAiCall(payload)
  } catch (error) {
...
function beginCallLog(createdAt: string, model: string, requestJson: string): number | null {
  getDb()
  try {
    return insertAiCall({
      createdAt,
...
export async function proxyChatCompletions(c: Context): Promise<Response> {
  ...
  const callId = beginCallLog(createdAt, model, requestJson)
