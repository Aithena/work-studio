...
export function insertAiCall(input: AiCallInsert): number {
...
export function listAiCalls(limit = 100): AiCallRow[] {
  ...
  const rows = getDb()
    ...
    .all(size) as DbAiCall[]
  return rows.map((row) => mapRow(row, false))
}

export function getAiCall(id: number): AiCallRow | null {
  const row = getDb()
    .prepare(
      `
      SELECT
