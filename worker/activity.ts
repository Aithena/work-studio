...
export async function listActivity(db: D1Database, days = 371): Promise<ActivityDay[]> {
  const end = new Date()
  ...
  const map = new Map((results ?? []).map((row) => [row.day, Number(row.count) || 0]))
...
export async function getMonthOverview(db: D1Database, date = new Date()): Promise<MonthOverview> {
...
export async function listOperationLogs(db: D1Database, limit = 100): Promise<OperationLogRow[]> {
